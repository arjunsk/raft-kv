...
import (
	...
	"encoding/json"
	"fmt"
	node2 "github.com/arjunsk/raft-kv/node"
	"io"
	"log"
	"net/http"
	"net/http/httptest"
...
func getLeaderNode() *node2.RStorage {
	raftNode := setupNode()
...
func setupNode() *node2.RStorage {
	...
	storage, err := node2.NewRStorage(&config)
	if err != nil {
		log.Panic(err)
	}
	return storage
...
func TestGetValue(t *testing.T) {
	...
	assert.Equal(t, "", raftNode.Get(testKey),
		"KV storage must be empty before the test")
